...
def new_game():
    ...
    for row in range(3):
        for column in range(3):
            button[row][column].config(text="",bg="yellow")
...
label=Label(text= player + " - turn" ,font=("Arial",30),fg="black")
label.pack(side="top")
reset_button=Button(text="Restart",font=("Arial",25),command=new_game)
reset_button.pack(side="bottom")
frame=Frame(wn)
frame.pack()
for row in range(3):
    for column in range(3):
        button[row][column]=Button(frame,text="",font=("Arial",25),bg="gold",width=5,height=2,
